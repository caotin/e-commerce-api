...
public interface UserRepository
  ...
  @Query(
      "select case when count(u) > 0 then true else false end from users u where u.deletedAt is not null and u.email = :email")
  boolean findActiveUserEmails(@Param(value = "email") String email);

  @Query(
      "select case when count(u) > 0 then true else false end from users u where u.deletedAt is not null and u.email = :name")
  boolean findActiveUserName(@Param(value = "name") String name);

  Page<UserEntity> findAllByDeletedAtIsNull(
